...
export default function ChatDashboard() {
  ...
  useEffect(() => {
    ...
    if (thread === "seoul" || thread === "samsung" || thread === "gachon") {
      setActiveThread(thread as "samsung" | "seoul" | "gachon");
    }
  }, [location.search]);
      ...
      >
            ...
            <div className="category-section">
              ...
              <ul className="space-y-2">
                <li
                  ...
                  data-active={activeThread === "gachon"}
                  onClick={() => setActiveThread("gachon")}
                >
                  <div className="text-sm font-medium">가천대 축제 제안</div>
                  <div className="truncate text-xs text-muted-foreground">3팀 라인업 추천 (댄스·힙합)</div>
                </li>
                <li
                  className="chat-item cursor-pointer rounded-md border border-border bg-muted/30 p-3 transition hover:bg-muted/40 ring-0 data-[active=true]:bg-muted/50 data-[active=true]:border-l-2 data-[active=true]:border-l-[hsl(var(--info))]"
        ...
        <section className="flex w-3/4 flex-col">
          ...
          <header className="flex items-center justify-between border-b border-border p-4">
            <h2 className="text-lg font-semibold">{activeThread === "samsung" ? "삼성전자 런칭쇼 제안" : activeThread === "gachon" ? "가천대 축제 제안" : "서울대 축제 제안"}</h2>
